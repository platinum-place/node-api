...
var createCase = async (data) => {
  const token = await generateAceessToken();
...
var getCase = async (id) => {
  const token = await generateAceessToken();

  const response = await fetch("https://zohoapis.com/crm/v2/Cases/" + id, {
    headers: {
      Authorization: "Zoho-oauthtoken " + token,
    },
  });
